...
const Mine = () => {
        ...
        if (FarmingDataStatus?.data?.farm?.createdAt && FarmingDataStatus?.data?.farm?.farmingEndTime) {
            ...
            intervalRef.current = setInterval(() => {
                ...
                if (secondsPassed >= totalFarmingTime) {
                    ...
                    if (document.getElementById('countdown')) {
                        document.getElementById('countdown')!.textContent = FarmingDataStatus?.data?.setting?.Mining_Rewards;
                    }
                }
            }, 1000);
    ...
    return (
        <div className="p-3 relative min-h-[85vh]">
            <div className="relative flex justify-center items-center w-full">
                <div className="bg-white bg-opacity-10 px-4 rounded-full font-poppins">
                    <span className="text-white text-opacity-80 font-ubuntu">Balance</span> <span className="text-white">{PointTableData?.data?.points?.point ? PointTableData?.data?.points?.point : 0}</span>
                </div>
                {/* <CiSettings className="text-white text-2xl absolute right-0" /> */}
            </div>
                ...
                PointTableLoading ?
                    <div className="flex justify-center items-center flex-col ">
                        ...
                        <div className="w-full flex items-center justify-between space-x-2 mt-2">
                            ...
                            <button onClick={() => ClaimFarming(FarmingDataStatus?.data?.farm?._id)} className={`w-[48%] px-4 py-3 flex items-center justify-center text-center rounded-[8px] font-semibold text-[14px] bg-white bg-opacity-15 text-[#888]${FarmingDataStatus?.data?.isPass === true && FarmingDataStatus?.data?.farm?.claim === false && 'bg-yellow-500 bg-opacity-100 text-black text-opacity-100'}`}>Claim</button></div>
                    </div>
                    :
                    <div className="flex justify-center items-center flex-col ">
                        ...
                        <div className="bg-white bg-opacity-15 w-full p-3 rounded-xl">
                            ...
                            <p className="pt-3 pb-1 font-medium text-[10px] text-[#93792b] flex items-center justify-center space-x-[2px]">
                                <img src="https://doghousesclonev01.vercel.app/starsorange.svg" alt="star" className="size-2" />
                                {FarmingDataStatus?.data?.setting?.Mining_Rewards} tokens profit per {time(FarmingDataStatus?.data?.setting?.Mining_Time)}
                            </p>
                        </div>
    ...
    ${FarmingDataStatus?.data?.isPass === true && FarmingDataStatus?.data?.farm?.claim === false
            ...
            <Link to={'/boosting'} className="w-full py-2 rounded-lg bg-white text-black font-poppins text-center mt-4 font-medium flex items-center gap-1 justify-center">
